...
- Code-quality pass (2026-06): applied safe, behavior-preserving fixes only — `useMemo` for derived lists (Accounts/Stock), `useCallback`-stabilized data loaders + eslint-disable on intentional mount effects (Dashboard/CreditCards/Accounts/Stock/Wholesale/AuthContext), `AuthContext` provider value memoized, extracted nested ternaries into named helpers, removed a `console.warn`, and env-ized test passwords. Verified via smoke test (all pages load, auth ok).
- Backend complexity refactor (2026-06, re-tested): `dashboard_summary()` split into pure helpers `_credit_metrics` / `_due_reminders` / `_profit_metrics`; `card_statement()` extracted `_build_statement`; `account_ledger()` + `card_ledger()` share a new `_ledger_rows()` helper. All verified byte-IDENTICAL via before/after JSON diff on each endpoint + full iteration5/6 suite green (17/17).
- Declined scanner items (false positives / regression risk): `data-testid` strings mis-flagged as "secrets"; `active is True` is correct for `Optional[bool]`; list-comprehension var `a` is scoped; localStorage→httpOnly cookie rearchitecture. Remaining optional: frontend component splits (CreditCards/CreditCardLedger/Accounts), `create_stock`/`record_transaction` — deferred (higher-risk, need careful re-test).
