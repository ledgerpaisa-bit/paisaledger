...
- Backend complexity refactor (2026-06, re-tested): `dashboard_summary()` split into pure helpers `_credit_metrics` / `_due_reminders` / `_profit_metrics`; `card_statement()` extracted `_build_statement`. Verified byte-IDENTICAL output via before/after JSON diff on both endpoints + full iteration5/6 suite green (17/17).
